...
app.config['UPLOAD_FOLDER'] = '../static/shorts'
...
def home():
   ...
   for short in old_shorts:
       try:
           os.remove(os.path.join(app.config['UPLOAD_FOLDER'], short.video_path))
       except:
           pass
   ...
   if request.method == 'POST' and 'video' in request.files and can_upload:
       ...
       if video and user_name:
           if video.filename.endswith('.mp4'):
               ...
               new_short = Short(video_path=filename, user_name=user_name)
